...
class SearchHomeVC: UITableViewController {
    ...
    static let storyboardName = "Searches"
    static let storyboardID = "searchHomeVC"
    ...
    func setupSearchField() {
        ...
        searchController.searchBar.barTintColor = Theme.colors().background
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 0:
            ...
            showResults(with: history.searchString)
        default: break
        }
        return
    }
    ...
    func showResults(with stringTag: String?) {
        if let stringTag = stringTag {
            SearchManager.main.appendSearch(SearchManager.SearchHistory(timeStamp: Date(), searchString: stringTag))
            tableView.reloadData()
        }
        ...
        dataSource.getResults(asNew: true, withTags: dataSource.tags(from: stringTag)).then {
...
class SearchHomeVCTableHeaderView: UIView {
    ...
    func setupLayout() {
        ...
        mainLabel.textColor = Theme.colors().text
        mainButton.tintColor = Theme.colors().link
    }
