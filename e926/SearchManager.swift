...
class SearchManager {
    ...
    private init() {
        if let storedHistory = UserDefaults.standard.array(forKey: SearchManager.defaultsKey) as? [String] {
            searches = storedHistory.map { SearchHistory(timeStamp: Date(), searchString: $0) }
        }
    }
    ...
    @objc(SearchManager_SearchHistory) class SearchHistory: NSObject, NSCoding {
        ...
        required convenience init?(coder aDecoder: NSCoder) {
            guard let timeStamp = aDecoder.decodeObject(forKey: "timeStamp") as? Date,
                ...
                //let sortFilter = aDecoder.decodeObject(forKey: "sortFilter") as? SortFilter
            else { return nil }
            
            self.init(timeStamp: timeStamp, searchString: searchString)
        }
        ...
        init(timeStamp: Date, searchString: String) {
